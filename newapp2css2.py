...
import io
from audio_recorder_streamlit import audio_recorder # Using the third-party component
...
HOSPITAL_LOGO_URL = "https://placehold.co/150x50/005A9C/FFFFFF/png?text=BCK+Logo&font=raleway" # White background logo
HOSPITAL_DETAILS = {
...
def transcribe_audio(model, audio_bytes, filename="recorded_audio"):
    """Transcribes audio bytes using the loaded Whisper model."""
    if model is None: return "Error: Transcription model not loaded."
    if audio_bytes is None: return "Error: No audio data received for transcription."
    audio_path = None
    try:
        suffix = os.path.splitext(filename)[1] if '.' in filename else ".wav"
        if suffix.lower() not in ['.wav', '.mp3', '.m4a', '.aac', '.ogg', '.flac']: suffix = ".wav"
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp_audio_file:
            tmp_audio_file.write(audio_bytes)
            audio_path = tmp_audio_file.name
        logging.info(f"Starting transcription for temporary file: {audio_path} (from {filename})")
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            except OSError as e: logging.error(f"Error removing temporary file {audio_path}: {e}")
...
def generate_summary_with_llm(structured_data, transcription, hospital_details):
    ...
    # --- REFINED SYSTEM PROMPT with MORE EXPLICIT Header Instruction ---
    system_prompt = f"""
...
- **Hospital Header (MANDATORY FIRST ELEMENT):** Start the document *immediately* with a header containing the hospital details provided (Name, Address, Phone, Logo URL) using Markdown. Format it clearly with the logo image tag first, then name, address, phone, followed by a horizontal rule. **CRITICAL: You MUST include the Markdown image tag `![Hospital Logo]({HOSPITAL_LOGO_URL})` at the very beginning.** Example structure:
`![Hospital Logo]({HOSPITAL_LOGO_URL})`
...
"""
    ...
    try:
        ...
        # Check if response includes expected structure (logo URL + hospital name + first section)
        if not summary or HOSPITAL_LOGO_URL not in summary or f"## {hospital_details['Name']}" not in summary or f"### {SUMMARY_SECTIONS_ORDERED[0]}" not in summary:
             logging.warning(f"LLM response might be invalid or missing structure: {summary[:300]}...")
             error_message = "Error: LLM generated an invalid or improperly formatted response (missing header/structure). Please review transcription and try again, or edit manually."
...
def main():
    ...
    <style>
        ...
        /* REMOVED generic stContainer styling that caused caption issues */
        .stCaption { color: #636e72; font-style: italic; margin-top: 0.5em; /* Adjusted margin */ margin-bottom: 1em; } /* General caption style */
        ...
        div[data-testid="stImage"] > img { max-height: 150px; object-fit: cover; margin-bottom: -10px; }
        /* Style for the preview markdown area */
        div.stMarkdown { /* Target markdown divs more generally within columns */
             height: 700px; overflow-y: auto; border: 1px solid #dfe6e9; border-radius: 8px; padding: 1rem; background-color: #f8f9fa;
        }
        /* Style audio recorder button - basic styling */
        div[data-testid="stAudioRecorder"] > div > button {
             border-radius: 8px !important;
             border: 1px solid #0073B7 !important; /* Blue border */
             background-color: #f0f8ff !important; /* Light blue background */
             color: #0073B7 !important; /* Blue text */
             padding: 0.5rem 1rem !important;
             margin-top: 0 !important; /* Align with subheader better */
             font-weight: bold !important;
        }
        div[data-testid="stAudioRecorder"] > div > button:hover {
             background-color: #d0e8f8 !important;
        }
        /* Recording state is handled visually by the component itself */
    </style>
    """, unsafe_allow_html=True)

    # --- Header Image ---
    if APP_HEADER_IMAGE_URL:
        st.image(APP_HEADER_IMAGE_URL, use_container_width=True)
    # --- END Header Image ---

    st.title("Discharge Summary AI Assistant")
    # REMOVED Caption below title
    ...
    st.header("🎙️ Upload or Record Audio & Generate Summary")
    ...
    if valid_visit_selected:
        ...
        with input_col1:
            ...
            uploaded_audio_file = st.file_uploader("Select audio file:", type=None, key="audio_uploader", label_visibility="collapsed")
        with input_col2:
            st.subheader("Record Audio")
            # Use audio_recorder - Use default appearance
            recorded_audio_bytes = audio_recorder(key="audio_recorder")
            st.caption("Click the icon to record/stop.") # Simple text indicator


        # Determine which audio source to use
        audio_source_bytes = None
        audio_source_name = "audio_recording.wav"
        audio_player_source = None

        if uploaded_audio_file is not None:
            audio_source_bytes = uploaded_audio_file.getvalue()
            audio_source_name = uploaded_audio_file.name
            audio_player_source = uploaded_audio_file
            # st.caption("Using uploaded file.") # Can be inferred
        elif recorded_audio_bytes is not None:
            audio_source_bytes = recorded_audio_bytes
            audio_player_source = recorded_audio_bytes
            # st.caption("Using recorded audio.") # Can be inferred

        # Display audio player if source exists
        if audio_player_source:
            st.audio(audio_player_source)

        # Process Button
        process_button_clicked = st.button("✨ Generate Summary", key="process_button", disabled=(audio_source_bytes is None), use_container_width=True)

        if st.session_state.validation_error:
            st.error(st.session_state.validation_error)

        if process_button_clicked and audio_source_bytes:
            # Reset state
            st.session_state.llm_summary = ""
            st.session_state.transcription = ""
            st.session_state.validation_error = None
            ...
            transcription_result = "Error: Could not process audio."
            if model:
                with st.spinner("Transcribing audio..."):
                    transcription_result = transcribe_audio(model, audio_source_bytes, filename=audio_source_name)
            else:
            ...
            st.rerun() # Rerun needed to display results/errors
        ...
        st.info("Please select a Patient and Visit before uploading or recording audio.")
    ...
    edit_value = st.session_state.get('edited_summary', '')
    if not edit_value and st.session_state.processing_complete and not st.session_state.validation_error and "error" not in st.session_state.get('llm_summary', '').lower():
    ...
    with edit_col:
        ...
        edited_summary_text = st.text_area(
            ...
            height=700,
            key="final_summary_edit_area", # Keep key consistent
        ...
        # Display caption directly below the text area in this column
        st.caption("Make final corrections here. The preview on the right will update.")
    ...
    with preview_col:
        ...
        if st.session_state.validation_error:
            preview_content = f"**Validation Error:**\n\n{st.session_state.validation_error}\n\n*Summary generation skipped.*"
             ...
             preview_content = '*Upload/Record audio and click "Generate Summary" first.*'
        elif "error" in st.session_state.get('llm_summary','').lower() and not current_edited_text:
             preview_content = f"*LLM generation failed:*\n\n{st.session_state.llm_summary}"
        elif not current_edited_text and st.session_state.processing_complete:
             preview_content = "*Summary generated by AI is in the editable area. Edit summary on the left to see preview.*"
        elif not current_edited_text:
             preview_content = "*Edit summary on the left to see preview.*"
        else:
             preview_content = current_edited_text

        # Display the live preview using markdown
        # Add a div with scrollbar for preview
        st.markdown(f"""
        <div style="height: 700px; overflow-y: auto; border: 1px solid #dfe6e9; border-radius: 8px; padding: 1rem; background-color: #f8f9fa;">
        {preview_content}
        </div>
        """, unsafe_allow_html=True)
        # Display caption directly below the markdown preview in this column
    ...
    if st.button("✅ Mark as Ready for Doctor Review", key="send_button", disabled=finalize_disabled, use_container_width=True):
        if valid_visit_selected:
            # In a real application, save `current_edited_text` content
            final_content_to_send = st.session_state.get('edited_summary', '') # Get final edits from state
