...
def validate_transcription_vs_selection(transcription, selected_patient_id, selected_patient_name):
    ...
    if mismatch:
        ...
        return True, "Validation passed (or no identifiers found/matched in audio)."
...
def main():
    ...
    with sel_col2:
        ...
        if st.session_state.current_patient_id:
            ...
            if not patient_visits.empty:
                ...
                if st.session_state.current_visit_id in visit_options.values():
                     current_visit_index = list(visit_options.values()).index(st.session_state.current_visit_id)
    ...
    if valid_visit_selected:
        ...
        if uploaded_audio is not None:
            ...
            if st.button("Transcribe and Generate Summary", key="process_button"):
    ...
    # Use columns for Transcription and AI Summary/Edit Area
    rev_col1, rev_col2 = st.columns(2)

    with rev_col1:
        st.subheader("Audio Transcription")
        st.text_area("", value=st.session_state.get('transcription', '*Transcription will appear here.*'), height=600, key="transcription_display_final", disabled=True, label_visibility="collapsed")

    with rev_col2:
        st.subheader("AI Generated / Editable Summary")

        # Determine initial value for edit area
        edit_value = ""
        display_summary = st.session_state.get('llm_summary', '')

        if st.session_state.validation_error:
            display_summary = f"*{st.session_state.validation_error}*\n\n*Summary generation skipped.*"
            edit_value = "" # Don't pre-fill edit area if validation failed
        elif not st.session_state.processing_complete and st.session_state.transcription and "error" in st.session_state.transcription.lower():
             ...
             display_summary = '*Upload audio and click "Transcribe and Generate Summary".*'
             edit_value = ""
        elif "error" in display_summary.lower():
             # If LLM returned an error, show error but allow editing from scratch
             edit_value = ""
        else:
             # Pre-fill edit area only on successful generation
             edit_value = display_summary

        # Display the AI output (or status message) using markdown
        summary_container = st.container(height=300, border=True) # Container for view-only AI output
        summary_container.markdown(display_summary if display_summary else '*Summary will appear here after processing.*', unsafe_allow_html=True)
        st.caption("Review the AI draft above. Make final corrections or additions in the text area below.")
    ...
    # --- NEW: Formatted Final Summary Preview ---
