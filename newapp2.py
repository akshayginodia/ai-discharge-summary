...
def validate_transcription_vs_selection(transcription, selected_patient_id, selected_patient_name):
    ...
    if name_in_audio or id_in_audio:
        ...
        if name_in_audio and selected_patient_name:
            # Use a more lenient check: allow if audio name is PART of selected name
    ...
    if mismatch:
        ...
        # Validation passes if no identifiers are found OR if found identifiers match
...
def main():
    ...
    with sel_col2:
        ...
        if st.session_state.current_patient_id:
            ...
            if not patient_visits.empty:
                ...
                # Check if the current visit ID is still valid for the selected patient
                ...
                if st.session_state.current_visit_id in valid_visit_ids:
                     ...
                     # If current visit ID is no longer valid (due to patient change), reset it
                     if st.session_state.current_visit_id is not None:
                         st.session_state.current_visit_id = None # Will default to index 0
                ...
                # Update state only if visit changed OR if it was previously None
                if st.session_state.current_visit_id != _selected_visit_id:
                    st.session_state.current_visit_id = _selected_visit_id
                    # Reset downstream states ONLY if visit actually changes
                    st.session_state.transcription = ""
                    st.session_state.llm_summary = ""
                    st.session_state.structured_data_for_llm = {}
                    st.session_state.validation_error = None
                    ...
                    # Prepare structured data immediately on visit change
                    try:
                    ...
                    # Rerun only if visit genuinely changed to update UI state
                    st.rerun()
                # If visit didn't change, ensure structured data is loaded if it's missing (e.g., on initial load)
                elif not st.session_state.structured_data_for_llm:
                     try:
                        patient_info = patients_df[patients_df['PatientID'] == st.session_state.current_patient_id].iloc[0]
                        visit_info = patient_visits[patient_visits['VisitID'] == st.session_state.current_visit_id].iloc[0]
                        visit_diagnoses = diagnoses_df[diagnoses_df['VisitID'] == st.session_state.current_visit_id]
            ...
            if process_button_clicked:
                ...
                if "error" not in transcription_result.lower():
                    ...
                    if is_valid:
                        ...
                        # Error will be displayed on rerun in the section above button
                        st.session_state.llm_summary = ""
                else:
    ...
    with edit_col1:
        st.subheader("AI Generated Draft / Final Preview")
        ...
        st.caption("This is the AI draft. Make final corrections in the 'Final Editable Summary' area.")
